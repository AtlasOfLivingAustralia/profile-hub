...
@Stepwise
@Slf4j
class PrivateImageFileSpec extends Specification implements ControllerUnitTest<ProfileController> {
...
//        controller.metaClass.request = mockRequest
    ...
    def "Thumbnails are made for private image"() {
        ...
        assert tilesDir.exists()
        and: "it contains tiles"
        TestCase.assertFalse("What happened to our thumbnails?", tilesDir.listFiles().size() == 0)
    }
    ...
    def "Private image thumbnail is retrieved"() {
        ...
        TestCase.assertNotNull("Image has not been retrieved ", thumbNail)
    ...
    def "File upload does not move the image file"(){
        ...
        TestCase.assertFalse("File has been moved unexpectedly",profileDir.listFiles().size() == 0)
    }
    ...
    def "A private image can be sent to central image location"() {
        ...
        TestCase.assertFalse(imageDir.exists())
